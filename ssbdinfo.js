...
const folderRelative = "icons/";

// Defined all icons
const starIcon = new L.icon({iconUrl: folderRelative + 'starIcon.png', className: 'customicon'});
const breakableIcon = new L.icon({iconUrl: folderRelative + 'breakable_wall_icon.png', className: 'customicon'});
const redKeycardIcon = new L.icon({iconUrl: folderRelative + 'red_keycard_icon.png', className: 'customicon'});
const greenKeycardIcon = new L.icon({iconUrl: folderRelative + 'green_keycard_icon.png', className: 'customicon'});
const blueKeycardIcon = new L.icon({iconUrl: folderRelative + 'blue_keycard_icon.png', className: 'customicon'});
const starRng = new L.icon({iconUrl: folderRelative + 'starIcon_rng.png', className: 'customicon'});
const easterEgg = new L.icon({iconUrl: folderRelative + 'easteregg.png', className: 'customicon'});
const secret = new L.icon({iconUrl: folderRelative + 'secret.png', className: 'customicon'});
...
let savedMarkedMarkers = localStorage.getItem("markedMarkers");
...
const bounds = [[-0.13824389122438063, 0.140625], [-234.04822155806622, 123.359375]];
...
let tileLayer = L.tileLayer("{z}\\{x}\\{y}.png", {
    noWrap: true,
    minZoom: 2,
    maxZoom: 8,
    center: [0,0],
...
map.on('click', function(ev){
    ...
    if(selectedMarkerIcon != undefined) {
        ...
        // add marker to list
        let pos = L.latLng([latlng.lat, latlng.lng]);
        const mark = new L.marker(L.latLng(pos), {icon: selectedMarkerIcon});
        mark.addTo(map);
        map.addLayer(mark);
